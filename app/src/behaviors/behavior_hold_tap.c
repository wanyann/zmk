...
static void decide_hold_tap(struct active_hold_tap *hold_tap,
    ...
    // otherwise the grace timer forces the tap. The window only opens when the
    // first other key was pressed before our own release (position_of_first_
    // other_key_pressed already set), so keys pressed during the window don't
    // matter — see position_state_changed_listener.
    ...
    if (hold_tap->grace_pending) {
        // Deferred resolution that finished the grace window: our own key was
        // already released, so complete the press with the matching release
        // (hold/tap) and free the slot. This runs for both a hold resolved by
        // the other key's release and a tap forced by the grace expiry.
...
static int position_state_changed_listener(const zmk_event_t *eh) {
    ...
    // Race with a grace window: our own key has been released and we are
    // waiting to see whether the trigger key that was already held gets
    // released quickly enough. Only the key-up of that exact trigger key can
    // resolve us to a hold; any other event (a new key-down, or the release of
    // some other key) is ignored — such keys were pressed during the window
    // and must not influence the decision.
    if (undecided_hold_tap->grace_pending) {
        if (undecided_hold_tap->config->hold_trigger_on_release) {
            // Release-based triggering isn't combined with the release grace.
            return ZMK_EV_EVENT_BUBBLE;
        }
